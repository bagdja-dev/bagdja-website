 ...
 * lihat order-hanlde-plan.md §3.2), tiap step yang punya `release_percentage`
 * dapat tombol "Setujui pelepasan dana" + "Ajukan Komplain".
 *
...
import type { OrderFulfillmentProgress, TransactionItem } from './order-detail-content';
...
export function FulfillmentProgress({
  ...
  const byFlow = new Map<
    string,
    { orderId: string; productName: string; progress: OrderFulfillmentProgress }[]
  >();
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const res = await fetch(
        `/api/transactions/${transactionId}/orders/${pending.orderId}/steps/${encodeURIComponent(pending.stepName)}/${path}`,
        { method: 'POST' },
      );
                ...
                <div key={gi.orderId}>
                  ...
                  <ol className="mt-2 flex flex-col gap-2">
                    {gi.progress.steps.map((step, index) => {
                      ...
                      const pendingAmount =
                        step.releasePercentage != null ? (step.releasePercentage / 100) * group.groupTotal : null;
                              ...
                              <div className="mt-2 flex flex-col gap-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  <button
                                    type="button"
                                    onClick={() =>
                                      setPending({ orderId: gi.orderId, stepName: step.stepName, kind: 'approve-release' })
                                    }
                                  ...
                                  {!step.disputed && (
                                    <button
                                      type="button"
                                      onClick={() =>
                                        setPending({ orderId: gi.orderId, stepName: step.stepName, kind: 'dispute' })
                                      }
                                      className="rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-wide transition-transform hover:scale-[1.02] active:scale-95"
