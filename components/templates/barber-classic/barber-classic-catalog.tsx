 ...
 * `CategoryGridSection` di barber-classic-view.tsx) navigasi ke halaman
 * listing ini. Duplikasi struktur dari `store-classic-catalog.tsx` (bukan
 * cross-import) — cuma styling yang beda (tema gelap + aksen amber).
 */
...
import { CatalogCard } from './barber-classic-view';
...
function CatalogHeading({ title }: { title: string }) {
  return (
    <div
      className="mb-6 flex items-center px-3 py-2 text-left text-xl font-semibold sm:text-2xl"
      style={{
        backgroundImage: 'linear-gradient(to right, var(--brand-accent), transparent)',
      ...
      {hasMore && tenantSlug && (
        <div className="mt-8 text-center">
          <button
            ...
            onClick={loadMore}
            disabled={loading}
            className="rounded-full px-6 py-2.5 text-sm font-medium transition-transform hover:scale-105 active:scale-95 disabled:opacity-60"
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      ...
      {items.length === 0 && !loading ? (
        <p style={{ color: 'var(--brand-muted)' }}>Belum ada item di kategori ini.</p>
      ) : (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-3 justify-items-center">
          {items.map((item) => (
            <CatalogCard key={item.id} item={item} websiteSlug={websiteSlug} />
  ...
  return (
    <div className="mt-6 flex flex-col items-center gap-4">
      {axisKeys.map((axisKey, axisIndex) => {
        ...
        return (
          <div key={axisKey}>
            <p className="mb-2 text-center text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--brand-muted)' }}>
              {axisKey}
