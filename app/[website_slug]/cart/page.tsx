/**
 * Halaman Cart (keranjang) — W1b/W2/W2.5 cart server-side draft.
 *
 * Server Component: load tenant + auth, lalu render lewat template `Renderer`
 * dengan section type `cart` — header/footer/theme konsisten dengan home &
 * halaman lain. Konten interaktif (fetch draft, qty stepper, hapus) di
 * `components/cart-content.tsx` (client). Route diprotect middleware
 * (wajib login buyer).
 */
...
export async function generateMetadata({ params }: CartPageProps): Promise<Metadata> {
  ...
  if (!tenant) return {};
  return { title: `Keranjang — ${tenant.website.name}` };
}
...
export default async function CartPage({ params }: CartPageProps) {
  ...
  const auth = await getAuthViewState(`${resolveTenantLinkBase(params.website_slug)}/cart`);
  ...
  const sections: SectionEntry[] = [
    { type: 'cart', content: { slug: params.website_slug } },
  ];
